...
class Test2Config : public Config {
public:
  int calcType;
  int cutPosType, cutBidType;
  size_t ndim[3];
  int treeLevel, dIndex;
  float org[3], d[3];
  char xmlFile[MAX_LEN];

  Test2Config(const char* file) : Config(file) 
  {
    ...
    if (xscanf("%d", &treeLevel) != 1) readError();
    if (xscanf("%d", &dIndex) != 1) readError();
